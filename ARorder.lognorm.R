...
#' 
#' @details
#' The log-normal TAR model is given by \deqn{log X_t=a_0^{(j)} + \sum_{i=1}^{k_j}a_i^{(j)}log X_{t-i}+h^{(j)}e_t} when \eqn{Z_t\in (r_{j-1},r_j]} for som \eqn{j} (\eqn{j=1,\cdots,l}). 
...
#' @param \eqn{\{Z_t\}} The threshold series
#' @param \eqn{\{X_t\}} The series of interest
#' @param \eqn{l} The number of regimes.
#' @param \eqn{r} The vector of thresholds for the series \{Z_t\}.
#' @param \eqn{k_Max} The minimum value for each autoregressive order. The default is 3.
#' @param \eqn{k_Min} The maximum value for each autoregressive order. The default is 0.
#' @param \eqn{n.sim} Number of iteration for the Gibbs Sampler
#' @param \eqn{p.burnin} Percentage of iterations used for 
#' 
#' @references 
...
ARorder.lognorm <- function(Z, X, l, r, k_Max=3, k_Min=0, n.sim=500, p.burnin=0.3, n.thin=1, plot=FALSE){
    if(length(r)!=(l-1))
