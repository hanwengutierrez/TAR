...
#' @param \eqn{r} The vector of thresholds for the series \eqn{\{Z_t\}}.
#' @param \eqn{K} The vector containing the autoregressive orders of the \eqn{l} regimes.
#' @param \eqn{theta} The matrix of autoregressive coefficients of dimension \eqn{l\times\max{K}}. \eqn{j}-th row contains the autoregressive coefficients of regime \eqn{j}.
#' @param \eqn{H} The vector containing the variance weights of the \eqn{l} regimes.
#' 
#' @references 
...
simu.tar.lognorm <- function(Z, l, r, K, theta, H){
  if(length(r)!=(l-1))
  ...
  for(i in (k+1):N){
    j <- J[i]
    X[i] <- exp(sum(c(1,log(X[i-(1:K[j])]))*theta[j,1:(K[j]+1)]) + H[j]*e[i])
  }
  X
}