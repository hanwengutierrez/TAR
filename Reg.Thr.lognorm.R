...
#' @param \eqn{p.burnin} Percentage of iterations used for Burn-in
#' 
#' @references 
...
reg.thr.lognorm <- function(Z, X, n.sim=500, p.burnin=0.2, n.thin=1, plot=FALSE){
  set.seed(123456789)
  l_Max=3
  l_Min=2
  k_Max=3
  k_Min=0
    ...
    estima.theta <- function(l,K,R){
      ...
      if(k>0) {
        ...
        for(t in (k+1):N){
          ...
          e[t]<-log(X[t])-a[1]-sum(a[-1]*c(log(X[(t-1):(t-K[jt])])))
        }
      }
    ...
    for(iii in 1:length(pos_R2)){
      ...
      nj <- table(J2)
      if(min(nj)>n.min.2&length(nj)==2){indicador<-c(indicador,iii)}
      Sys.sleep(0.1)   
    ...
    for(iii in 1:dim(pos_R3)[1]){
      ...
      nj <- table(J3)
      if(min(nj)>n.min.3&length(nj)==3){indicador<-c(indicador,iii)}
      Sys.sleep(0.1)   
